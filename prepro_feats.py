...
def main(params):
    ...
    for i,img in enumerate(imgs):
        ...
        np.save(os.path.join(dir_fc, str(img['filename'][:-4])), tmp_fc.data.cpu().float().numpy())
        np.savez_compressed(os.path.join(dir_att, str(img['filename'][:-4])), feat=tmp_att.data.cpu().float().numpy())
